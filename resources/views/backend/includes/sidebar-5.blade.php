...
<div class="col-md-3 col-lg-2 sidebar-offcanvas bg-light pl-0" id="sidebar" role="navigation" style="min-height: calc(100vh - 55px);">
    <ul class="nav flex-column sticky-top pl-0 pt-6 mt-4 posfix">
            <li class="nav-item">
            ...
            @if((!$logged_in_user->hasRole('student')) && ($logged_in_user->hasRole('teacher') || $logged_in_user->isAdmin() || $logged_in_user->hasAnyPermission(['course_access','lesson_access','test_access','question_access','bundle_access'])))
                ...
                @can('item_access')
                    <li class="nav-item ">
                        <a class="nav-link {{ $request->segment(2) == 'items' ? 'active' : '' }}"
                           href="{{ route('admin.items.index') }}">
                            <i class="nav-icon icon-layers"></i>
                            <span class="title">@lang('menus.backend.sidebar.items.title')</span>
                        </a>
                    </li>
                @endcan
